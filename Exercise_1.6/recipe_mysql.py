...
# Main Menu function - loops until user types quit
...
# Main menu option 2 - create new recipe
...
# Main menu search recipes function
def search_recipe(conn, cursor):
    ...
    # show user all ingredients in every recipe
    ...
    try:
        # user input which index number ingredient they want to search for 
    ...
    # search in the tables rows in the ingredient column for the ingredient specified 
    else:
    ...
    # display recipe data once found
...
def update_recipe(conn, cursor):
    ...
    if column_for_update == "name":
        ...
        # re calculate difficulty for updated recipes
        ...
        cursor.execute("UPDATE Recipes SET difficulty = %s WHERE id = %s",
                       (updated_difficulty, recipe_id_for_update))
        print("Modification done.")
    # commit updates changes to the table
    conn.commit()
